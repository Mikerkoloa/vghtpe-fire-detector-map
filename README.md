...
- 提供本機解析的「查詢助手」，可輸入 `M6-55`、`身障 M6-55`、`長青B3 55 73` 等句型，整理成可開啟並圈選的結果卡片。
- 支援 PDF 放大、縮小、適合寬度、清除標記、儲存圈選圖片、列印目前頁面、開啟原始 PDF。
- 儲存圈選圖片在電腦會直接下載 PNG；手機或平板會開啟圖片預覽，並優先使用系統分享儲存。
- 列印功能不需要先圈選；列印頁預設 A3 橫式，若目前頁面有圈選標記會一起印出。
- 支援滑鼠滾輪縮放、左鍵拖曳移動圖面、雙擊圈選標記放大，手機可用兩指縮放並在放大後單指拖曳。
- 頁面滑到下方後會顯示右下角「回到上方」按鈕，可快速回到搜尋與棟別區。
- 版面支援桌機、平板、手機。
...
```text
https://vghtpe-fire-detector-map.vercel.app/admin.html
```
