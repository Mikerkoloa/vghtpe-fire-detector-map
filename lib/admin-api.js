...
const SETTINGS_PATH = "data/app-settings.json";
...
function loadAppSettings() {
  try {
    return mergeAppSettings(readJson(SETTINGS_PATH));
  } catch {
...
function sanitizePdfFileName(fileName) {
  return path.basename(String(fileName || "").normalize("NFKC").trim()).replace(/[\\/]/g, "");
}
...
function expectedPdfFileName(buildingName, floorLabel) {
  if (!buildingName || !floorLabel) return "";
  return `${buildingName}${floorLabel} YYYYMMDD.pdf`;
}

function matchesPdfNamingRule(buildingName, floorLabel, fileName) {
  const safeFileName = sanitizePdfFileName(fileName);
  if (!buildingName || !floorLabel || !safeFileName) return true;
  const escapedName = buildingName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const escapedFloor = floorLabel.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp(`^${escapedName}${escapedFloor}\\s+\\d{8}\\.pdf$`, "i").test(safeFileName);
}
...
function findPdfItem(data, payload) {
  ...
  if (pathValue) {
    const byPath = data.pdfItems.find((item) => item.path === pathValue);
    if (byPath) return byPath;
  }

  return data.pdfItems.find((item) => item.building === buildingName && item.floor === floorLabel) || null;
}
...
function findTarget(payload, data) {
  ...
  const uploadedPath = buildUploadedFilePath(buildingName, payload.file?.name);
  const fallbackPath = buildingName && floorLabel ? buildFallbackPath(buildingName, floorLabel) : "";
  const pathFromFloor = floor?.path || fallbackPath;
  const nextPath = uploadedPath || pathFromFloor || normalizeText(payload.path);
...
function validateUploadPayload(payload, data) {
  ...
  if (file.name && !matchesPdfNamingRule(target.buildingName, target.floorLabel, file.name)) {
    warnings.push(`建議檔名格式：${expectedPdfFileName(target.buildingName, target.floorLabel)}`);
  }

  const duplicatedPath = data.pdfItems?.find((item) => item.path === target.path && (item.building !== target.buildingName || item.floor !== target.floorLabel));
  if (duplicatedPath) {
    errors.push(`目標檔名已被 ${duplicatedPath.building} ${duplicatedPath.floor} 使用`);
  }
...
function buildUpdatedHistoryData(payload, target, commitLabel) {
  ...
  const pathIndex = files.findIndex((item) => item.path === target.path);
  const oldPathIndex = target.oldPath && target.oldPath !== target.path ? files.findIndex((item) => item.path === target.oldPath) : -1;
  const previousIndex = pathIndex >= 0 ? pathIndex : oldPathIndex;
  ...
  const nextFiles = files.filter((_, index) => index !== pathIndex && index !== oldPathIndex);
  const insertAt = previousIndex >= 0 ? Math.min(previousIndex, nextFiles.length) : nextFiles.length;
  nextFiles.splice(insertAt, 0, nextRecord);
...
function buildUpdatedBuildingData(target, mode) {
  const buildingData = readJson("data/buildings.json");
  const buildings = buildingData.buildings || [];
  let building = buildings.find((item) => item.name === target.building);
  ...
  if (!building) return buildingData;

  let floor = building.floors.find((item) => item.label === target.floor);
  if (!floor && mode === "create") {
    floor = {
      label: target.floor,
      fileId: "",
      path: target.path,
...
function validateHistoryCorrectionPayload(payload, data) {
  const pdfItem = findPdfItem(data, payload);
  const errors = [];

  if (!pdfItem) errors.push("找不到要修正的 PDF 圖面");
  if (!payload.updatedAt) errors.push("請填寫更新日期");
  if (!payload.updatedBy?.trim()) errors.push("請填寫更新人員");
  if (!payload.commitMessage?.trim()) errors.push("請填寫 Commit 訊息");
  ...
  return {
    ...
    target: pdfItem
      ? {
          building: pdfItem.building,
          floor: pdfItem.floor,
          path: pdfItem.path,
          detectorCount: pdfItem.detectorCount || 0,
          pageCount: pdfItem.pageCount || 0,
        }
      : null,
  };
}

function buildCorrectedHistoryData(payload, target, commitLabel) {
  const historyData = readJson("data/pdf-update-history.json");
  const files = historyData.files || [];
  const index = files.findIndex((item) => item.path === target.path);
  const previous = index >= 0 ? files[index] : null;
  const note = payload.note?.trim() || "修正更新紀錄";
  const nextEntry = {
    date: payload.updatedAt,
  ...
  const nextRecord = {
    ...
    updatedBy: payload.updatedBy,
    note,
    detectorCount: previous?.detectorCount || target.detectorCount || 0,
    pageCount: previous?.pageCount || target.pageCount || 0,
    history: [nextEntry, ...(previous?.history || [])],
  };
  const nextFiles = files.filter((_, fileIndex) => fileIndex !== index);
  const insertAt = index >= 0 ? Math.min(index, nextFiles.length) : nextFiles.length;
  nextFiles.splice(insertAt, 0, nextRecord);
...
function mockCorrectHistory(payload) {
  const data = loadAdminData();
  const result = validateHistoryCorrectionPayload(payload, data);
  ...
  const historyData = buildCorrectedHistoryData(payload, result.target, "mock-pending");
  return {
    ok: true,
    mode: "mock",
    message: "本機未設定 GitHub env，已完成模擬修正，尚未寫入 GitHub",
    ...
    historyRecord: historyData.files.find((item) => item.path === result.target.path),
  };
}

function normalizeRecentQueryLimit(value) {
...
function validateSettingsPayload(payload) {
  const errors = [];
  const recentQueryLimit = normalizeRecentQueryLimit(payload.assistant?.recentQueryLimit);

  if (recentQueryLimit === null) errors.push("最近查詢筆數必須是 0 到 50 的整數");
  if (!payload.commitMessage?.trim()) errors.push("請填寫 Commit 訊息");
  ...
  return {
    ...
    settings: mergeAppSettings({
      generatedAt: new Date().toISOString(),
      assistant: {
        recentQueryLimit: recentQueryLimit ?? DEFAULT_APP_SETTINGS.assistant.recentQueryLimit,
      },
    }),
  };
}

function mockUpdateSettings(payload) {
  ...
  return {
    ok: true,
    mode: "mock",
    message: "本機未設定 GitHub env，已完成模擬儲存，尚未寫入 GitHub",
...
async function uploadToGithub(payload) {
  ...
  const buildingData = buildUpdatedBuildingData(target, result.mode);
  ...
  const [pdfBlobSha, historyBlobSha, buildingBlobSha] = await Promise.all([
    createGithubBlob(config, content.contentBase64, "base64"),
    createGithubBlob(config, `${JSON.stringify(historyData, null, 2)}\n`, "utf-8"),
    createGithubBlob(config, `${JSON.stringify(buildingData, null, 2)}\n`, "utf-8"),
  ]);
  const treeItems = [
    {
      path: target.path,
...
async function correctHistoryOnGithub(payload) {
  const config = getGithubConfig();
  if (!githubEnvReady()) {
    if (isVercelRuntime()) {
      return {
    ...
    return mockCorrectHistory(payload);
  }
  ...
  const branch = config.branch || payload.branch || "main";
  const historyData = buildCorrectedHistoryData(payload, result.target, "same-commit");
  const ref = await githubFetch(config, `/git/ref/heads/${encodeURIComponent(branch)}`);
  const parentSha = ref.object.sha;
  const parentCommit = await githubFetch(config, `/git/commits/${parentSha}`);
  const historyBlobSha = await createGithubBlob(config, `${JSON.stringify(historyData, null, 2)}\n`, "utf-8");
  const tree = await githubFetch(config, "/git/trees", {
    method: "POST",
    body: JSON.stringify({
      base_tree: parentCommit.tree.sha,
  ...
  const finalHistoryData = buildCorrectedHistoryData(payload, result.target, commit.sha);

  return {
    ok: true,
    mode: "github",
    message: "已修正更新紀錄並寫入 GitHub",
    target: result.target,
    commit: {
      branch,
      message: payload.commitMessage,
    ...
    historyRecord: finalHistoryData.files.find((item) => item.path === result.target.path),
  };
}

async function updateSettingsOnGithub(payload) {
  const config = getGithubConfig();
  if (!githubEnvReady()) {
    if (isVercelRuntime()) {
      return {
  ...
  const parentSha = ref.object.sha;
  const parentCommit = await githubFetch(config, `/git/commits/${parentSha}`);
  const settingsBlobSha = await createGithubBlob(config, `${JSON.stringify(result.settings, null, 2)}\n`, "utf-8");
  const tree = await githubFetch(config, "/git/trees", {
    method: "POST",
    body: JSON.stringify({
      base_tree: parentCommit.tree.sha,
...
async function handleNodeRequest(req, res, action) {
  try {
    ...
    if (action === "settings") {
      ...
      if (req.method === "POST") {
        const result = await updateSettingsOnGithub(await collectJson(req));
    ...
    if (action === "history") {
      ...
      if (!requireAdmin(req, res)) return;
      const result = await correctHistoryOnGithub(await collectJson(req));
      sendJson(res, result.ok ? 200 : 400, result);
      return;
    }
...
module.exports = {
  ...
  mockUpdateSettings,
  preflightUpload,
  correctHistoryOnGithub,
  updateSettingsOnGithub,
  uploadToGithub,
  loadAppSettings,
};
