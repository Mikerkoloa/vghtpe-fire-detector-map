...
- 雙擊圈選標記放大。
- 清除標記。
- 列印目前 PDF 頁面；沒有圈選也可列印，若目前頁面有圈選標記會一起印出。
- 開啟原始 PDF。
...
- 若使用者要求正式發版，除了 commit/push，也應建立並推送 Git tag，例如 `git tag v0.2.8 && git push origin v0.2.8`。
...
- `npm run check:version`：確認 `v0.2.8` 已同步到主頁、管理後台與 `CHANGELOG.md`。
